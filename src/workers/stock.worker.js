    ...
    conn.createChannel(function (err, ch) {
        ...
        ch.assertQueue('', {
            ...
            ch.consume(queue.queue, async function(msg) {
                const operationMsg = msg.fields.routingKey  === 'incremented' ? 'Incrementar' : 'Decrementar'
                const productName = String.fromCharCode(...msg.content.slice(1, msg.content.length -1));
                const { Product } = db;
                ...
                if(productToUpdate) {
                    ...
                    if(operationMsg === 'Incrementar') {
                        ...
                        quantity = productToUpdate.dataValues.quantity > 0 ? (productToUpdate.dataValues.quantity - 1) : 0
                    }
